...
def get_admin_or_teacher(user_id: int = Depends(get_user_id_from_token), db: Session = Depends(get_db)) -> Optional[LmsUsers]:
    user = db.query(LmsUsers).filter(LmsUsers.user_id == user_id).first()
    if user is None:
        raise HTTPException(status_code=404, detail="User not found")
...
def get_admin_student_teacher(user_id: int = Depends(get_user_id_from_token), db: Session = Depends(get_db)) -> Optional[LmsUsers]:
    user = db.query(LmsUsers).filter(LmsUsers.user_id == user_id).first()
    if user is None:
        raise HTTPException(status_code=404, detail="User not found")
...
def get_admin_student_teacher_parent(user_id: int = Depends(get_user_id_from_token), db: Session = Depends(get_db)) -> Optional[LmsUsers]:
    user = db.query(LmsUsers).filter(LmsUsers.user_id == user_id).first()
    if user is None:
        raise HTTPException(status_code=404, detail="User not found")
...
def get_current_user(token: str = Depends(JWTBearer()), db: Session = Depends(get_db)) -> Optional[LmsUsers]:
    try:
        payload = decodeJWT(token)
        if payload:
...
def is_admin(current_user: Optional[LmsUsers] = Depends(get_user_id_from_token)):
